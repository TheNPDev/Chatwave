...
//    } else {
//
//        val statuses: List<Status> by viewModel.status.observeAsState(listOf())
//
...
//                                    }
//                                    CommonDivider()
//                                    val uniqueUsers = otherStatuses?.map { it.user }?.toSet()
//                                        ?.toList()  ?: emptyList()
...
fun StatusScreen(navController: NavController, viewModel: CWViewModel) {
    ...
    val statuses by viewModel.status.observeAsState(emptyList())
    val userData = viewModel.userData.value
